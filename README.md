...
### v1.2.0 (2026.05.18)
- ✨ 채팅방에서 사진 전송 기능 추가
- ✨ 자신이 보낸 채팅 수정, 삭제 기능 추가
- ✨ 친구 프로필 메모 기능 추가
- ✨ 채팅방, 친구 목록, 랭킹 탭에서 프로필을 눌러 다른 유저 프로필 확인 가능
- ✨ 목표 페이지에 날짜별 진행 목표 개수와 완료율 표시
- ✨ 홈 화면에 오늘의 목표 완료율 표시
- 🔧 랭킹탭에 친구가 아닌 유저도 표시되도록 변경 — 친구가 아닌 유저의 달성 목표는 비공개 처리
- 🔧 채팅방 이름 변경 기능을 그룹채팅에만 적용 가능하도록 변경
- 🔧 게시판 탭 일기 탭으로 명칭 변경
- 🔧 목표 수정 시, 최초 생성할 때 입력했던 내용을 자동으로 불러오도록 수정
- 🔧 반복 목표를 수정할 경우, 한 개만 수정할지 다른 반복 목표도 모두 수정할지 선택하도록 변경
- 🔧 목표 생성 시 알림을 추가하면 목표가 생성되지 않거나 비정상적으로 많이 생성되는 문제 해결
- 🔧 기존 버전 사용자가 업데이트 이후 앱을 재실행할때마다 로그인을 해야하는 현상 해결
...
### v1.1.1 (2026.05.12)
- ✨ 공휴일, 대체 공휴일 API 조회 기능 추가
- 🔧 공휴일 날짜 글꼴 색상을 빨간색으로 변경
