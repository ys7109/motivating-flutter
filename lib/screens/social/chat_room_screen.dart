...
class _ChatRoomScreenState extends State<ChatRoomScreen> {
  ...
  // 1:1 채팅 읽음 여부 확인 — 상대방 lastReadAt이 메시지 작성 시각 이후면 읽은 것으로 판단
  ...
  void _syncMemberProfiles(List<String> memberUids) {
    ...
    // 퇴장 멤버는 구독 해제하지 않음 — 이전 메시지의 프로필 표시용 (5번)
  }
              ...
              if (widget.isGroup)
                ...
                GestureDetector(
                  ...
                  child: CharacterAvatar(
                    ...
                    size: 36,
                    profileImageUrl: otherUid.isNotEmpty ? _profileImageOf(otherUid) : null,
                  ),
                ),
              ...
              builder: (context, snap) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    // 2번: 연속 메시지에서 시간이 같으면 마지막(다음 메시지가 다른 사람이거나 시간이 다를 때)만 시간 표시
                    final nextIdx = idx + 1;
                    final hasNext = nextIdx < messages.length;
                    final nextMsg = hasNext ? messages[nextIdx] : null;
                    // 다음 메시지가 같은 사람이고 시간 문자열이 같으면 현재 메시지 시간 숨김
        ...
        return Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            ListTile(
              ...
              title: Text('채팅방 이름 변경', style: TextStyle(
            ...
            if (widget.isGroup)
              ListTile(
                ...
                title: Text('참여자 보기', style: TextStyle(
                    color: context.textPrimary, fontWeight: FontWeight.w500)),
            ...
            if (widget.isGroup)
              ListTile(
                ...
                title: Text('멤버 추가', style: TextStyle(
                    color: context.textPrimary, fontWeight: FontWeight.w500)),
            ...
            ListTile(
              ...
              title: const Text('채팅방 나가기', style: TextStyle(
        ...
        return Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('참여자 ${members.length}명', style: TextStyle(fontSize: 16,
              ...
              child: ListView.separated(
                ...
                itemBuilder: (_, i) {
                  ...
                  return Padding(
                    ...
                    child: Row(children: [
                      CharacterAvatar(character: _characterOf(uid), size: 38,
                          profileImageUrl: _profileImageOf(uid)),
                      const SizedBox(width: 10),
                      Expanded(child: Text(_displayNameOf(uid), style: TextStyle(
                          fontSize: 14, fontWeight: FontWeight.w500, color: ctx.textPrimary))),
                      if (uid == _myUid)
                        Text('나', style: TextStyle(fontSize: 12, color: ctx.textSecondary)),
                    ]),
                  );
                },
              ),
  ...
  Future<void> _showAddMemberDialog(BuildContext context) async {
    ...
    final friends = friendDocs.where((d) => d.exists)
        .map((d) => {'uid': d.id, ...d.data()!}).toList();
    if (!mounted) return;
                ...
                return GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Row(children: [
                      ...
                      if (isSelected)
                        Icon(Icons.check_circle, size: 18, color: ctx.primaryColor),
                    ]),
                  ),
                );
              }),
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Center(child: Text('추가하기', style: TextStyle(
                    ...
                    color: selected.isEmpty ? ctx.textSecondary : ctx.onPrimary,
                  ))),
                ),
              ),
            ]),
...
class _MessageBubble extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // 시스템 메시지 — 중앙 안내 문구로 표시 (3번: OO님이 나갔습니다)
    ...
    return Padding(
      // 연속 메시지(isContinued)는 간격 좁힘, 반응 있으면 4, 기본 7, 연속은 2
        ...
        children: [
          if (isMe)
            Row(
              ...
              children: [
                // 2번: hideTime이면 읽음/시간 숨김
                if (!hideTime)
                  Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                    if (isGroup && groupUnreadCount > 0)
                      // 그룹 채팅 미읽음 수신자 수 표시
                      Text('$groupUnreadCount', style: TextStyle(
                          fontSize: 10, color: context.primaryColor, fontWeight: FontWeight.w600))
                ...
                if (!hideTime) const SizedBox(width: 4),
                // 내 메시지 버블
                Container(
                  ...
                  child: Text(message.content,
                      style: TextStyle(fontSize: 14, height: 1.4, color: context.onPrimary)),
                ),
              ],
            )
          else
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(99),
                        border: Border.all(
                          color: isMine ? context.primaryColor.withOpacity(0.4) : context.borderColor,
                        ),
                      ),
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        Text(e.key, style: const TextStyle(fontSize: 13)),
                        const SizedBox(width: 3),
                        Text('${e.value.length}', style: TextStyle(
                            fontSize: 11, color: context.textSecondary, fontWeight: FontWeight.w500)),
                      ]),
                    ),
                  );
                }).toList(),
