        ...
        children: [
              ...
              child: Row(children: [
                ...
                GestureDetector(
                  ...
                  child: Container(
                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                    decoration: BoxDecoration(color: context.primaryColor, borderRadius: BorderRadius.circular(99)),
                    child: Text('수락', style: TextStyle(fontSize: 12, color: context.isDark ? Colors.black : Colors.white)),
                  ),
                ),
              ]),
            )),
            ...
            Row(children: [['total', '누적'], ['daily', '오늘'], ['average', '평균']].map((t) {
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  child: Text(t[1], style: TextStyle(fontSize: 11, color: isSelected ? (context.isDark ? Colors.black : Colors.white) : context.textSecondary)),
                ),
              );
            }).toList()),
          ]),
          ...
          if (_friendRankings.isEmpty)
            Padding(
              ...
              child: Center(child: Text('친구를 추가하면 랭킹이 표시돼요', style: TextStyle(fontSize: 13, color: context.textSecondary))),
            )
          else
            ..._friendRankings.map((user) {
              final isMe = user['uid'] == myUid;
              final medal = user['rank'] == 1 ? '🥇' : user['rank'] == 2 ? '🥈' : user['rank'] == 3 ? '🥉' : null;
              final focusMin = _rankTab == 'total' ? user['totalFocusMin'] : _rankTab == 'daily' ? user['todayFocusMin'] : user['avgFocusMin'];
              return Container(
                margin: const EdgeInsets.only(bottom: 8),
                padding: const EdgeInsets.all(12),
                decoration: BoxDecoration(
          ...
          if (_friends.isEmpty)
            ...
            ..._friends.map((friend) => _FriendTile(friend: friend, onRemove: () => _removeFriend(friend['uid']))),
...
class _FriendTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String lastSeenText = '접속 정보 없음';
    if (isOnline) {
      lastSeenText = '접속 중';
    } else if (lastSeen != null) {
      final dt = lastSeen.toDate();
      final diff = DateTime.now().difference(dt as DateTime);
      if (diff.inMinutes < 1) lastSeenText = '방금 전 접속';
      else if (diff.inMinutes < 60) lastSeenText = '${diff.inMinutes}분 전 접속';
      else if (diff.inHours < 24) lastSeenText = '${diff.inHours}시간 전 접속';
      else lastSeenText = '${diff.inDays}일 전 접속';
    }
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            const SizedBox(width: 6),
            Text(lastSeenText, style: TextStyle(fontSize: 12, color: isOnline ? const Color(0xFF4CAF50) : context.textSecondary)),
          ]),
        ])),
        GestureDetector(onTap: onRemove, child: Icon(Icons.person_remove_outlined, size: 20, color: context.textSecondary)),
      ]),
    );
  }
}
