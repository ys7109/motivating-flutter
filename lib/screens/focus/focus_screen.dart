...
class _FocusScreenState extends State<FocusScreen> {
  ...
  double get _progress => _totalSec > 0 ? (_totalSec * 1000 - _elapsedMs) / (_totalSec * 1000) : 0.0;
  ...
  int get _toNextBonus { final nextMin = ((_elapsed ~/ 60) ~/ 10 + 1) * 10; return nextMin * 60 - _elapsed; }
  int get _nextBonusXp => ((_elapsed ~/ 60) ~/ 10 + 1) * 10;
  bool get _isActive => _phase == 'running' || _phase == 'paused' || _phase == 'paused_by_switch';
  ...
  void _syncTotal() {
    if (_phase == 'idle') setState(() { _totalSec = _hInput * 3600 + _mInput * 60 + _sInput; _elapsedMs = 0; });
  }
  ...
  void _tick() {
    ...
    _timer = Timer.periodic(const Duration(milliseconds: 16), (_) {
      if (!mounted) return;
      final diffMs = DateTime.now().difference(_startTime!).inMilliseconds;
      final newElapsedMs = _elapsedAtStartMs + diffMs;
      if (newElapsedMs >= _totalSec * 1000) { setState(() => _elapsedMs = _totalSec * 1000); _onDone(); return; }
  ...
  void _applyAddTime() {
    ...
    _timer?.cancel();
    _elapsedAtStartMs = _elapsedMs;
    setState(() { _totalSec += add; _addH = 0; _addM = 0; _addS = 0; _addPopup = false; if (_phase == 'done') _phase = 'running'; });
                ...
                child: Column(children: [
                  Align(
                    ...
                    child: TextButton.icon(
                      onPressed: () { if (_isActive) setState(() => _exitPopup = true); },
                      icon: Icon(Icons.arrow_back_ios, size: 14, color: context.textSecondary),
                      label: Text('홈으로', style: TextStyle(fontSize: 14, color: context.textSecondary)),
                      style: TextButton.styleFrom(padding: EdgeInsets.zero, minimumSize: Size.zero),
                    ),
                  ),

                  Text('집중 모드', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: context.textPrimary)),
                  const SizedBox(height: 4),
                  Text('시간을 설정하고 집중을 시작하세요', style: TextStyle(fontSize: 14, color: context.textSecondary)),
                  if (_statusMsg.isNotEmpty) ...[
                    const SizedBox(height: 2),
                    Text(_statusMsg, textAlign: TextAlign.center, style: TextStyle(fontSize: 12, color: context.textSecondary, height: 1.5)),
                  ],
                  ...
                  SizedBox(
                    ...
                    child: Stack(fit: StackFit.expand, children: [
                      CustomPaint(painter: _RingPainter(
                        progress: _phase == 'idle' ? 1.0 : _progress,
                        phase: _phase, r: r,
                      ...
                      Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                        Text(_fmt(_remaining), style: TextStyle(
                          ...
                          fontWeight: FontWeight.w300, color: context.textPrimary,
                          letterSpacing: -2, fontFeatures: const [FontFeature.tabularFigures()],
                        )),
                        if (_isActive) ...[
                          const SizedBox(height: 4),
                          Text('$elMin분 집중 완료', style: TextStyle(fontSize: 13, color: context.textSecondary)),
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Row(mainAxisSize: MainAxisSize.min, children: [
                                Text('+', style: TextStyle(fontSize: 14, color: context.textSecondary)),
                                const SizedBox(width: 4),
                                Text('시간 추가', style: TextStyle(fontSize: 12, color: context.textSecondary)),
                              ]),
                            ),
                          ),
                        ],
                  ...
                  if (_phase == 'idle') ...[
                    Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      _TimeInput(label: '시간', value: _hInput, max: 23, onChanged: (v) { setState(() => _hInput = v); _syncTotal(); }),
                      Padding(padding: const EdgeInsets.only(top: 18), child: Text(':', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w300, color: context.textSecondary))),
                      _TimeInput(label: '분', value: _mInput, max: 59, onChanged: (v) { setState(() => _mInput = v); _syncTotal(); }),
                      Padding(padding: const EdgeInsets.only(top: 18), child: Text(':', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w300, color: context.textSecondary))),
                      _TimeInput(label: '초', value: _sInput, max: 59, onChanged: (v) { setState(() => _sInput = v); _syncTotal(); }),
                    ]),
                    ...
                    Row(
                      ...
                      children: _presets.map((p) {
                        ...
                        return GestureDetector(
                          onTap: () { setState(() { _hInput = p['h'] as int; _mInput = p['m'] as int; _sInput = p['s'] as int; }); _syncTotal(); },
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Text('현재까지 ', style: TextStyle(fontSize: 13, color: context.textSecondary)),
                    Text('+$_realtimeXp XP', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: context.textPrimary)),
                    Text(' 획득 예정', style: TextStyle(fontSize: 13, color: context.textSecondary)),
                  ]),

                  if (_isActive && _elapsed > 0) ...[
                    const SizedBox(height: 8),
                    Text('다음 +$_nextBonusXp XP 보너스까지 ${_fmt(_toNextBonus)}${_phase == 'paused' ? ' ⏸' : ''}',
                        style: TextStyle(fontSize: 12, color: context.textSecondary), textAlign: TextAlign.center),
                  ],
                  const SizedBox(height: 16),

                  if (_phase == 'idle') _BigBtn(label: '집중 시작', disabled: _totalSec == 0, onTap: _start),

                  if (_isActive) ...[
                    _BigBtn(label: _phase == 'paused' || _phase == 'paused_by_switch' ? '다시 집중' : '일시정지',
                  ...
                  if (_phase == 'done') ...[
                    Container(
                      ...
                      child: Column(children: [
                        ...
                        const SizedBox(height: 8),
                        Text('집중 완료!', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: context.textPrimary)),
                        const SizedBox(height: 4),
                        Text('$elMin분 집중 → +$_realtimeXp XP 획득', style: TextStyle(fontSize: 13, color: context.textSecondary)),
                      ]),
                    ),
                    const SizedBox(height: 10),
                  ...
                  Container(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('XP 획득 기준', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: context.textPrimary)),
                      const SizedBox(height: 8),
                      Text('1분마다 1 XP를 획득하며, 매 10분마다 현재까지 집중한 시간만큼 XP를 추가 획득합니다.',
                          style: TextStyle(fontSize: 13, color: context.textSecondary, height: 1.7)),
                      const SizedBox(height: 10),
                      Container(
                        ...
                        child: Text('예) 10분 집중 시 10 XP 추가 획득\n     50분 집중 시 50 XP 추가 획득',
                            style: TextStyle(fontSize: 13, color: context.textSecondary, height: 1.8)),
                      ),
                      const SizedBox(height: 10),
                      Text('* 화면을 끄거나 잠금 상태에서도 타이머가 계속 동작합니다.\n* 다른 앱으로 이동하면 타이머가 일시정지됩니다.',
                          style: TextStyle(fontSize: 11, color: context.textSecondary, height: 1.6)),
                    ]),
                  ),
                ]),
              ),
...
class _RingPainter extends CustomPainter {
  ...
  const _RingPainter({required this.progress, required this.phase, required this.r, required this.activeColor, required this.bgColor});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (phase == 'idle') {
      canvas.drawCircle(center, r, Paint()..color = activeColor..style = PaintingStyle.stroke..strokeWidth = strokeW);
      return;
    }

    canvas.drawCircle(center, r, Paint()..color = bgColor..style = PaintingStyle.stroke..strokeWidth = strokeW);

    if (progress > 0) {
      final color = phase == 'done' ? const Color(0xFF1b8a5a)
          : phase == 'paused' || phase == 'paused_by_switch' ? const Color(0xFF9E9E9E)
          : activeColor;
      canvas.drawArc(Rect.fromCircle(center: center, radius: r), -pi / 2, 2 * pi * progress, false,
          Paint()..color = color..style = PaintingStyle.stroke..strokeWidth = strokeW..strokeCap = StrokeCap.round);
    }
  }

  @override
  bool shouldRepaint(_RingPainter old) => old.progress != progress || old.phase != phase;
}
...
class _TimeInputState extends State<_TimeInput> {
  ...
  void didUpdateWidget(_TimeInput old) {
    ...
    if (!_focused && old.value != widget.value) _ctrl.text = '${widget.value}';
  }
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(children: [
        Text(widget.label, style: TextStyle(fontSize: 11, color: context.textSecondary)),
        const SizedBox(height: 4),
        Focus(
          onFocusChange: (f) {
            setState(() => _focused = f);
            if (!f) { final n = (int.tryParse(_ctrl.text) ?? 0).clamp(0, widget.max); _ctrl.text = '$n'; widget.onChanged(n); }
          },
          child: SizedBox(
            width: 70,
            child: TextField(
              ...
              style: TextStyle(fontSize: 28, fontWeight: FontWeight.w300, color: context.textPrimary),
              decoration: InputDecoration(
                ...
                focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide(color: context.primaryColor, width: 1.5)),
                filled: true,
                fillColor: context.surfaceColor,
              ),
              onChanged: (s) { final n = int.tryParse(s) ?? 0; if (n > widget.max) { _ctrl.text = '0'; widget.onChanged(0); } else widget.onChanged(n); },
            ),
          ),
        ),
      ]),
...
class _OutlineBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 15),
      decoration: BoxDecoration(border: Border.all(color: context.borderColor), borderRadius: BorderRadius.circular(99)),
      child: Center(child: Text(label, style: TextStyle(fontSize: 15, color: context.textSecondary))),
    ),
  );
}
      ...
      child: Center(child: Padding(
        ...
        child: Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Text('집중 종료', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: context.textPrimary)),
            const SizedBox(height: 8),
            Text('$elMin분 집중했어요.\n지금 종료하면 +$xp XP를 획득합니다.',
                textAlign: TextAlign.center, style: TextStyle(fontSize: 13, color: context.textSecondary, height: 1.6)),
            const SizedBox(height: 24),
            Row(children: [
              Expanded(child: GestureDetector(onTap: onCancel,
                child: Container(padding: const EdgeInsets.symmetric(vertical: 13),
                  decoration: BoxDecoration(color: context.subtleBg, borderRadius: BorderRadius.circular(12)),
                  child: Center(child: Text('계속 집중', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: context.textPrimary)))))),
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('시간 추가', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: context.textPrimary)),
                const SizedBox(height: 6),
                Text('추가할 시간을 입력하세요', style: TextStyle(fontSize: 13, color: context.textSecondary)),
                const SizedBox(height: 20),
                Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  _TimeInput(label: '시간', value: addH, max: 23, onChanged: onChangeH),
                  Padding(padding: const EdgeInsets.only(top: 18), child: Text(':', style: TextStyle(fontSize: 20, color: context.textSecondary))),
                ...
                if (hasAdd) ...[
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(color: context.subtleBg, borderRadius: BorderRadius.circular(12)),
                    child: Column(children: [
                      Text('추가 후 남은 시간', style: TextStyle(fontSize: 12, color: context.textSecondary)),
                      const SizedBox(height: 4),
                      Text(formatTime(preview), style: TextStyle(fontSize: 22, fontWeight: FontWeight.w500, color: context.textPrimary)),
                    ]),
                  ),
                ],
                const SizedBox(height: 20),
                Row(children: [
                  Expanded(child: GestureDetector(onTap: onCancel,
                    child: Container(padding: const EdgeInsets.symmetric(vertical: 13),
                      decoration: BoxDecoration(color: context.subtleBg, borderRadius: BorderRadius.circular(12)),
                      child: Center(child: Text('취소', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: context.textPrimary)))))),
                  const SizedBox(width: 10),
